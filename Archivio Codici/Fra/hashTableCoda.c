...
char *searchHash(char *string, int w, int h, int quality, int colorSpace){
   ...
   char *toHash = malloc(4*sizeof(int) + 4*sizeof(char));
   ...
   */
   ...
   //PROBLEMA: controllo dimensioni
   limite_dimensione+=imageSize;
   while(limite_dimensione>dimensione_max){
    libera_n(testa,testa->ind);
   }
